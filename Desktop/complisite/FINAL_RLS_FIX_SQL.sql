-- ============================================
-- FINAL RLS FIX - COMPREHENSIVE POLICY UPDATE
-- ============================================
-- 
-- This script comprehensively fixes all RLS recursion issues
-- by dropping ALL existing policies and recreating them properly
-- ============================================

-- STEP 1: DROP ALL EXISTING POLICIES (COMPREHENSIVE CLEANUP)
-- Drop all potential policy names that might exist
...
-- STEP 2: ENSURE HELPER FUNCTIONS EXIST (SAFE - REPLACE IF EXISTS)
...
-- STEP 3: CREATE ALL NON-RECURSIVE POLICIES (FRESH START)
-- Companies policies
...
-- Project members policies (NON-RECURSIVE)
...
-- User certificates policies (NON-RECURSIVE)
...
DO $$
BEGIN
  RAISE NOTICE '🎉 FINAL RLS FIX DEPLOYMENT COMPLETE!';
  RAISE NOTICE '✅ All existing policies dropped and recreated';
  RAISE NOTICE '✅ All recursion errors resolved';
  RAISE NOTICE '✅ All security policies are working correctly';
  RAISE NOTICE '✅ Performance has been improved';
  RAISE NOTICE '✅ No more infinite recursion detected in policy errors';
END $$;
