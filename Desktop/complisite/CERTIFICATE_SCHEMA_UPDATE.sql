-- ============================================
-- CERTIFICATE MANAGEMENT SCHEMA UPDATE
-- ============================================
-- 
-- This script updates the certificate tables to support the new
-- structured certificate management system with categories and types
-- ============================================

-- Update certificates table with new fields
...
INSERT INTO certificate_types (category_id, name, description, is_renewable, requires_training, typical_validity_months) VALUES
  -- Safety Training
...
CREATE INDEX IF NOT EXISTS idx_certificates_user_category ON certificates(user_id, category);
CREATE INDEX IF NOT EXISTS idx_certificate_types_category ON certificate_types(category_id);
...
SELECT 
  ...
  END as status
FROM certificates c
LEFT JOIN certificate_categories cc ON c.category = cc.name
LEFT JOIN certificate_types ct ON c.type = ct.name AND c.category = cc.name
LEFT JOIN issuing_organizations io ON c.issuing_organization = io.name;
...
CREATE POLICY "Anyone can read certificate types" ON certificate_types
  FOR SELECT USING (true);
